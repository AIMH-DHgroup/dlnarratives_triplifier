...
import narra.triplifier.TriplifyMoreJsons;
...
public class OWLOntologyPopulator2Nuova {
	...
	public OWLOntologyPopulator2Nuova() {
			...
			for (int j = 0; j < eventList.size(); j++) {
				...
				try {
					...
					if(!eventPolygon.isEmpty()) {
							...
							if(TriplifyMoreJsons.LAUS.get(eventPolygon)!=null){
								IRI_polygon = Vocabulary.base + "place/" + TriplifyMoreJsons.LAUS.get(eventPolygon);
								IRI_polygon_geometry = Vocabulary.base + "geometry/" + TriplifyMoreJsons.LAUS.get(eventPolygon);
							} else{
								IRI_polygon = Vocabulary.base + "place/" + TriplifyMoreJsons.countIdLAU;
								IRI_polygon_geometry = Vocabulary.base + "geometry/" + TriplifyMoreJsons.countIdLAU;
								TriplifyMoreJsons.LAUS.put(eventPolygon, TriplifyMoreJsons.countIdLAU);
								TriplifyMoreJsons.countIdLAU++;
							}
						// add the triple E5 Event, P7_took_place_at, SP2 Phenomenal Place
						addObjectPropertyAssertion(model, eventURI, Vocabulary.ecrm+Vocabulary.ecrmNames.P7_took_place_at.toString(),
						IRI_polygon, 
					...
					if(narrative.getPlace() != null) {
						...
						if(TriplifyMoreJsons.LAUS.get(narrative.getPlace())!=null){
							IRI_LAUPlace = Vocabulary.base + "place/" + TriplifyMoreJsons.LAUS.get(narrative.getPlace());
							IRI_LAUGeometry = Vocabulary.base + "geometry/" + TriplifyMoreJsons.LAUS.get(narrative.getPlace());
						} else{
							IRI_LAUPlace = Vocabulary.base + "place/" + TriplifyMoreJsons.countIdLAU;
							IRI_LAUGeometry = Vocabulary.base + "geometry/" + TriplifyMoreJsons.countIdLAU;
							TriplifyMoreJsons.LAUS.put(narrative.getPlace(), TriplifyMoreJsons.countIdLAU);
							System.out.print(TriplifyMoreJsons.countIdLAU);
							TriplifyMoreJsons.countIdLAU++;
						}

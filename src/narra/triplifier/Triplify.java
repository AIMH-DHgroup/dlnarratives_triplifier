...
public class Triplify {
	...
	public static void main(String[] args) throws Exception {
		...
		if (jsonFile.isFile()) {
			// Caso: singolo file JSON
			filesToProcess = new File[] { jsonFile };
		} else {
			// Caso: directory
			File dir = new File(jsonPath);
			filesToProcess = dir.listFiles((d, name) -> name.toLowerCase().endsWith(".json"));

			if (filesToProcess == null) {
				throw new RuntimeException("Directory non valida o accesso negato: " + jsonPath);
			}
		}

		// Ciclo comune per entrambi i casi
		for (File child : filesToProcess) {
			String fileName = child.getName();
			int dotIndex = fileName.lastIndexOf('.');

			if (dotIndex == -1) continue; // Niente estensione → skip

			String nameWithoutExt = fileName.substring(0, dotIndex);
			// owlPath = "./OWL/" + nameWithoutExt + ".owl";

			// Carica JSON
			Narrative narrative = JsonToJava.loadJSON(child.getAbsolutePath());
		...
		try {
			...
			if (result > 0) {
				...
				RDFXMLDocumentFormat rdfDocFormat = ("true".equalsIgnoreCase(properties.getProperty("expandOntology")))
						...
						//local= "https://tool.dlnarratives.eu/fuseki/narratives"; 
						//local= "https://tool.dlnarratives.eu/fuseki/violtwin"; 
						...
						//conn.delete("https://tool.dlnarratives.eu/adminNarra.1_N3261");
